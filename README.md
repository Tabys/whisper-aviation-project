...
> **Важно:** Обученная модель в репозитории **не хранится** (размер ~2 GB). После дообучения она сохраняется локально в папку `./whisper-aviation-model-medium-final/`. Базовая модель `openai/whisper-medium` скачивается автоматически с Hugging Face при запуске обучения.
